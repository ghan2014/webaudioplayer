'use strict';

/**
 * @ngdoc directive
 * @name webPlayerMiniAppApp.directive:wpMeta
 * @description
 * # wpMeta
 */
angular.module('webPlayerMiniAppApp')
    .directive('wpMeta', function (Themer) {
    	console.log('----wpmeta directive');
        return {
            restrict: 'E', // directive only for use as html element
            transclude: true, // we want to allow contents to have access to scope above us if it wants (do not contain contents in our scope)
            link: function (scope, element) {
            	console.log('---- wpmeta link');
                Themer.showTheme(scope, element, 'meta');
            }
        };
    });
